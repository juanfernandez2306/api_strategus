...
class ResetPasswordController
{
    ...
    // Instancia el validador estructural
    }

    public function __invoke(Request $request, Response $response): Response
    {
        $method = $request->getMethod();
// Enrutamiento interno por verbo HTTP
...
// Bloqueo preventivo en la UI si el enlace está roto
        $token = $queryParams['token'] ?? '';
        $email = $queryParams['email'] ?? '';
        if (empty($token) || empty($email)) {
            return $this->renderMessagePage($response, 'Enlace inválido', 'Los parámetros requeridos para la recuperación están incompletos.', 'error', 400);
        }
        ...
        if (!$dbToken || !hash_equals($dbToken['token'], hash('sha256', $token)) || strtotime($dbToken['expires_at']) < time()) {
            return $this->renderMessagePage($response, 'Enlace Inválido o Expirado', 'El enlace de recuperación ya no es válido, ya fue utilizado o ha caducado. Solicita una nueva recuperación.', 'error', 400);
        }

        // 2. OBTENER LOS DATOS REALES DESDE LA BASE DE DATOS
        $usuario = $this->repository->findByEmail($email);
        $nombreCompleto = 'Usuario GEPAD';
...
// Concatenamos el nombre y apellido devueltos por la sentencia SQL
        ...
        // Sobreescribimos la inferencia por split('@') utilizando el nombre y apellido real de tu DB
            $htmlContent = str_replace("const nombreInferido = emailParam.split('@')[0].replace(/[^a-zA-Z]/g, ' ');", "const nombreInferido = '{$nombreCompleto}';", $htmlContent);
        } else {
            return $this->renderMessagePage($response, 'Error de Sistema', 'El formulario de restablecimiento no fue encontrado en el servidor.', 'error', 500);
        }
...
// 1. Capturar los datos del payload del formulario

        // 2. Ejecutar validación Rakit
        $errores = $this->validator->validate($body);
        if (!empty($errores)) {
            return $this->renderMessagePage($response, 'Error de Validación', 'Los datos enviados no cumplen con los requisitos mínimos de seguridad o las contraseñas no coinciden.', 'error', 400, "<a href='javascript:history.back()' class='btn btn-error'>← Regresar al formulario</a>");
        }
...
// 3. Buscar el token activo en la base de datos
        $dbToken = $this->repository->getVerificationToken($email);
        if (!$dbToken) {
            return $this->renderMessagePage($response, 'Enlace Inválido', 'El enlace es inválido o ya ha sido utilizado anteriormente.', 'error', 440);
        }

        // 4. Validar integridad criptográfica (hash_equals)
        if (!hash_equals($dbToken['token'], hash('sha256', $tokenParam))) {
            return $this->renderMessagePage($response, 'Token Inválido', 'El token de seguridad proporcionado no es válido o fue alterado.', 'error', 400);
        }

        // 5. Validar expiración de tiempo (2 horas límite)
        if (strtotime($dbToken['expires_at']) < time()) {
            return $this->renderMessagePage($response, 'Enlace Expirado', 'El enlace ha expirado. Por favor, solicita una nueva recuperación.', 'error', 410);
        }
...
// 7. Guardar en Base de Datos vía transacción en el repositorio
        ...
        if (!$success) {
            return $this->renderMessagePage($response, 'Error de Servidor', 'Ocurrió un error interno al intentar procesar la solicitud. Por favor intente más tarde.', 'error', 500);
        }

        // 8. Mensaje de éxito visual final adaptado para Mobile-First
        return $this->renderMessagePage($response, '¡Contraseña Actualizada!', 'Tu nueva clave de acceso ya está activa. Puedes cerrar esta pestaña de forma segura y abrir la aplicación para iniciar sesión con normalidad.', 'success', 200);
    }
